  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        try {
          const categoryResponse = await api.get(`/products/getCategoriesById/${categoryId}`);
          setCategory(categoryResponse.data);
        } catch (categoryErr) {
          console.error("Failed to fetch category details:", categoryErr);
        }
...
try {
  ...
  const fetchProductsWithFilter = async () => {
    try {
      ...
      // Always filter by the main category ID
      if (categoryId) {
  ...
  category: p.category?.name,  // ✅ ADD THIS
        price: p.price,
        mrp: p.price + 200,
        discount: 20,
      ...
      try {
        const fallbackResponse = await api.get(`/products/by-category/${categoryId}`);
...
//       // 🧠 Save product safely (survives refresh)
        ...
        <>
          ...
          {products.length > 0 && (
            <div className="lg:hidden poppins-regular px-4 mt-4">
              ...
              >
                Filter & Sort {(selectedBrands.length > 0 || selectedConcerns.length > 0) ? 
                  `(${selectedBrands.length + selectedConcerns.length})` : ''}
              </button>
            </div>
          )}
          ...
          <div className="max-w-7xl mx-auto poppins-regular px-4 py-10 flex gap-8">
            ...
            <div className={`${products.length > 0 ? 'flex-1' : 'w-full'}`}>
              ...
              {(selectedBrands.length > 0 || selectedConcerns.length > 0 || minPrice > 0 || maxPrice < 10000) && (
